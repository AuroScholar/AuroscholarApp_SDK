package com.yugasa.yubokotlinsdk.service

import android.content.Context
import android.util.Log
import com.yugasa.yubokotlinsdk.BuildConfig.BASE_URL
...
//    private val queue: RequestQueue? = null
    ...
    fun createFormdata() {
        try {
            ...
            apiInterface = RetroBase.getClient(apiUrl!!)
                ?.create(ApiInterface::class.java) // Create retro fit client instans and Api and interface
            disposable!!.add(
                apiInterface!!.getLoginResponse(apiMethod, requestBodyMap)
                    ...
                    .subscribeWith(object : DisposableSingleObserver<Any>() {
                        override fun onSuccess(o: Any) {
                            ...
                            resultObj = AppUtil.getCustomGson()
                                ?.fromJson(AppUtil.getCustomGson()?.toJson(o), resultType)
                            if (listener != null) {
                                listener!!.onResult(
                                    apiUrl,
                                    apiMethod,
                        ...
                        override fun onError(e: Throwable) {
                            try {
                                AppUtil.LogMsg(
                                    "responseJson",
                                    methodUrl + "  " + AppUtil.getCustomGson()?.toJson(e)
                                )
                                ...
                                resultObj = AppUtil.getCustomGson()
                                    ?.fromJson(jsonObject.toString(), resultType)
                                if (listener != null) {
                                    listener!!.onResult(
                                        apiUrl,
                                        apiMethod,
    ...
    fun createJsondata() {
        try {
            ...
            if (apiMethod != null && apiMethod!!.isNotEmpty()) {
                methodUrl = apiUrl + apiMethod
            }
            apiInterface = RetroBase.getClient(apiUrl!!)
                ?.create(ApiInterface::class.java) // Create retro fit client instans and Api and interface
            AppUtil.LogMsg(
                TAG,
                "request: " + methodUrl + "  " + AppUtil.getCustomGson()!!.toJson(paramObj)
            )
            disposable!!.add(
   ...
   /*fun createMultipartFile() {
        try {
            ...
            AppUtil.LogMsg(
                TAG,
                "request: " + methodUrl + "  " + requestBodyMap!!
            )
            disposable!!.add(
                apiInterface!!.uploadWithSingleImage(apiMethod!!, file!!, requestBodyMap!!)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribeWith(object : DisposableSingleObserver<Any?>() {
                        override fun onSuccess(o: Any) {
                            AppUtil.LogMsg(
                                "responseJson",
                                "$BASE_URL$apiMethod  $requestBodyMap"
                            )
                            resultObj = AppUtil.getCustomGson()
                                ?.fromJson(AppUtil.getCustomGson()?.toJson(o), resultType)
                            if (listener != null) {
    ...
    fun createFormdataWithImage(userId: RequestBody, strApi: RequestBody) {
        try {
            apiInterface = RetroBase.getClient(apiUrl!!)!!
                .create(ApiInterface::class.java) // Create retro fit client instans and Api and interface
            if (apiUrl != null) {
            ...
            if (null != apiMethod && apiMethod!!.isNotEmpty()) {
                methodUrl = apiUrl + apiMethod
            }
            Log.e(TAG, "request: " + methodUrl + "  " + AppUtil.getCustomGson()!!.toJson(paramObj))
            disposable!!.add(
                apiInterface!!.uploadWithSingleImage(apiMethod!!, file!!, userId!!, strApi!!)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribeWith(object : DisposableSingleObserver<Any?>() {
                        override fun onSuccess(o: Any) {
                            AppUtil.LogMsg(
                                "responseJson",
                                "$BASE_URL$apiMethod  " + AppUtil.getCustomGson()!!
                                    .toJson(o)
                            )
                            resultObj = AppUtil.getCustomGson()!!
                                .fromJson(AppUtil.getCustomGson()!!.toJson(o), resultType)
                        ...
                        override fun onError(e: Throwable) {
                            try {
                                ...
                                val jsonObject = JSONObject(AppUtil.getCustomGson()!!.toJson(e))
                                resultObj = if (jsonObject.optInt("code") == 401) {

                                    jsonObject.put("message", "Invalid Credentials")
                                    jsonObject.put("status_code", "201")
                                    AppUtil.getCustomGson()!!.fromJson(jsonObject.toString(), resultType)
                                } else {
                                    val jsonObject1 = JSONObject()

                                    jsonObject.put("message", "Something went wrong")
                                    jsonObject.put("status_code", "201")
                                    AppUtil.getCustomGson()!!.fromJson(jsonObject1.toString(), resultType)
                                }
                                if (listener != null) {
                                    listener!!.onResult(
                                        apiUrl,
